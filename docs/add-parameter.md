...
1) Il faut configurer le moteur de recherche grâce à un bean de type "fr.ans.afas.fhirserver.search.config.domain.ServerSearchConfig"
...
La configuration précédente ne fait que la configuration système de stockage et de recherche de afas. Pour exposer l'api fhir en 
http, nous utilisons Hapi qui s'occupera de la partie controller. 


Créez une classe qui implémente IResourceProvider et optionnellement étends AsBaseResourceProvider (l'extension va faciliter certaines opérations).
...
Si tout ce passe bien, vous devriez avoir en appelant l'url `http://localhost:8080/fhir/v1` une réponse valide (sans résultats de recherche) : 
...
L’exposition et le parsing des paramètres sont gérés par la librairie HAPI FHIR. Le stockage et le moteur de recherche sont gérés par l’implémentation spécifique du projet.
...
Il faut ajouter le paramètre sur la méthode annotée avec @Search() de la classe qui implémente ca.uhn.fhir.rest.server.IResourceProvider pour la ressource FHIR souhaitée.

Par exemple pour Organization il s’agit de la classe com.synodis.fhir_server.provider.RassOrganizationProvider du projet fhir-server. 
...
Cela va avoir pour action de déclarer le paramètre dans le CapabilityStatement du serveur « /fhir/metadata?_format=json&_pretty=true » :
...
Une fois le paramètre récupéré, il faut le transmettre au système de requête. Cela se fait toujours dans la méthode précédente :
...
Le premier paramètre de cette méthode est un FhirSearchPath qui permet de définir la ressource sur laquelle appliquer le paramètre par rapport à la configuration du moteur de recherche.
...
En java, pour ajouter un paramètre, il faut ajouter le paramètre dans la configuration du bean qui implémente "fr.ans.afas.fhirserver.search.config.domain.ServerSearchConfig" :
