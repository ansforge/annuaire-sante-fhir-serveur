...
import fr.ans.afas.utils.data.TestSearchConfigService;
...
class FhirRequestParserTest {
    ...
    @Test
    void testSelectExpressionParsing() throws BadSelectExpression, BadDataFormatException {
        var expression = FhirRequestParser.parseSelectExpression("FhirResource?_count=49&string_path:exact=bla,bla&string_path:exact=blo", expressionFactory, searchConfigService);
    ...
    @Test
    void testSelectExpressionParsingIncludes() throws BadSelectExpression, BadDataFormatException {
        var expression = FhirRequestParser.parseSelectExpression("Organization?_include=", expressionFactory, searchConfigService);
        Assert.assertEquals(0, expression.getIncludes().size());

        expression = FhirRequestParser.parseSelectExpression("Organization?_include=Organization:partof", expressionFactory, searchConfigService);
        Assert.assertEquals(1, expression.getIncludes().size());
        List<String> includes = expression.getIncludes().stream().map(i -> i.getType() + ":" + i.getName()).toList();
        Assert.assertTrue(includes.contains("Organization:partof"));

        expression = FhirRequestParser.parseSelectExpression("PractitionerRole?_include=PractitionerRole:partof&_include=PractitionerRole:organization", expressionFactory, searchConfigService);
        Assert.assertEquals(2, expression.getIncludes().size());
        includes = expression.getIncludes().stream().map(i -> i.getType() + ":" + i.getName()).toList();
        ...
        expression = FhirRequestParser.parseSelectExpression("PractitionerRole?_include=*", expressionFactory, searchConfigService);
    ...
    @Test
    void testSelectExpressionParsingRevIncludes() throws BadSelectExpression, BadDataFormatException {
        var expression = FhirRequestParser.parseSelectExpression("Organization?_revinclude=", expressionFactory, searchConfigService);
        Assert.assertEquals(0, expression.getIncludes().size());

        expression = FhirRequestParser.parseSelectExpression("Organization?_revinclude=PractitionerRole:organization", expressionFactory, searchConfigService);
        Assert.assertEquals("Organization", expression.getFhirResource());
        Assert.assertEquals(1, expression.getRevincludes().size());
        List<String> includes = expression.getRevincludes().stream().map(i -> i.getType() + ":" + i.getName()).toList();
        Assert.assertTrue(includes.contains("PractitionerRole:organization"));

        expression = FhirRequestParser.parseSelectExpression("Organization?_revinclude=Device:organization", expressionFactory, searchConfigService);
        Assert.assertEquals("Organization", expression.getFhirResource());
        Assert.assertEquals(1, expression.getRevincludes().size());
        includes = expression.getRevincludes().stream().map(i -> i.getType() + ":" + i.getName()).toList();
        Assert.assertTrue(includes.contains("Device:organization"));

        expression = FhirRequestParser.parseSelectExpression("Organization?_revinclude=PractitionerRole:organization&_revinclude=Device:organization", expressionFactory, searchConfigService);
        Assert.assertEquals("Organization", expression.getFhirResource());
    ...
    void testSelectExpressionParsingIncludesThrowsException(String path) throws BadDataFormatException {
        Assert.assertThrows(BadSelectExpression.class,
                () -> FhirRequestParser.parseSelectExpression(path, expressionFactory, searchConfigService));
    }
    ...
    void testSelectExpressionParsingRevIncludesThrowsException(String path) throws BadDataFormatException {
        Assert.assertThrows(BadSelectExpression.class,
                () -> FhirRequestParser.parseSelectExpression(path, expressionFactory, searchConfigService));
    }
    ...
    @Test
    void testDateParsing() throws BadSelectExpression, BadDataFormatException {
        var expression = FhirRequestParser.parseSelectExpression("FhirResource?_count=49&date_path=eq2013,eq2023&date_path=gt2013-05-01", expressionFactory, searchConfigService);
    ...
    @Test
    void testUrlDecodeEmpty() throws BadSelectExpression, BadDataFormatException {
        // when the string is null:
        Assert.assertNull(FhirRequestParser.urlDecode(null));
        // when the string have no parameters:
        var expression = FhirRequestParser.parseSelectExpression("FhirResource", expressionFactory, searchConfigService);
